...
But "it works" was established on the models of a year ago, and every part of a harness is a bet about what the model *can't* do — bets that age as the model improves (a point I'll come back to). Three of them are aging badly right now, and all three bite hardest on exactly the k10s failure.
...
What's wrong is deferred. You can produce code that passes every test, survives every review, and ships looking great while being one feature interaction away from breaking — which is precisely how k10s broke: not a failed test, a feature interaction that took out live updates across views. Fragile code doesn't announce itself. It works, and works, and works, until the day the structure can't absorb the next change — and by then the god object has tangled everything together, so the break is brutal to localize and brutal to fix. The bill for the fragility comes due long after the review that waved it through.
...
And even the behavioral half doesn't necessarily need an *orchestration* harness. arch-gate ships its behavioral disciplines (test-driven development, systematic debugging, rigorous review, brainstorm-before-building) as in-context prose you reference from `CLAUDE.md`, plus two deterministic checks for things a model shouldn't be trusted to eyeball: `require_tests` (the deterministic shadow of TDD, ratcheted so legacy untested code is grandfathered and only new modules must ship a test) and `arch mutate`, mutation testing that flips an operator and re-runs your suite to prove the tests you have actually *kill bugs* rather than pad a count. Both are zero-token-deterministic; mutation testing is heavy, so it runs out-of-band in CI, not on the commit hot path. You get the whole quality stack, mechanical floor plus behavioral disciplines, with no role fan-out and no per-round token tax.
...
The gate carries dead weight too, and it deserves the same knife. Some of its checks are also bets on the model: `max_function_lines` is a cheap proxy for complexity, and `require_tests` plus the shipped in-context disciplines bet the model won't reliably do TDD unasked. If a future model writes well-factored, well-tested code on its own, those earn retirement exactly like a role does — don't grandfather your own scaffolding just because it's cheap to run.
...
## The cost curves point opposite directions

That audit is the cost curve made concrete. A harness costs **O(tokens × rounds × model-price)** and sheds value with every release — full freight for a quality lift that shrinks. A deterministic gate costs **~0 per commit**, and the part of it that's load-bearing is exactly the part the audit can't retire. The thing watching your architecture should be the one input to this whole system that *doesn't* get more expensive and less necessary over time.

That's the architecture I'd actually run: the deterministic floor on the hot path, blocking rot on every commit for free; the harness (or just a strong model with in-context disciplines) reserved for the behavioral half and run when the work genuinely needs an oracle.
