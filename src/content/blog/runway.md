...
This post is about a different problem domain. Runway is a vendor-neutral capacity-planning tool for AI compute (Apache-2.0; not yet on public GitHub — source available on request) — answering "when does declared demand outrun deployed capacity, what's slipping, and what intervention helps most?" Different stakeholders (financial-planning leadership, capacity PMs, service VPs), different math (bootstrap CI + empirical-Bayes slip priors + Monte Carlo runway distributions), different language stack (Python rather than TypeScript), and at v1 a different substrate from the inference-reliability bundle.
