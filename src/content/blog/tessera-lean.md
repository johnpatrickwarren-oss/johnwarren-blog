...
The result was not new mathematics. Every theorem in the chain was already known. What the prover produced instead was an audit finding: five of our formal statements of that known mathematics were wrong, and the extensive numerical validation we ran on each of them had passed every one.

Disclosures first, because they shape how to read everything below. Tessera is my system, and the proofs verify statements I have a stake in. The formal statements were originally written by Claude sessions transcribing our research notes into Lean, and the proofs that later exposed them were also done with Claude — the same kind of agent wrote the bugs and found them, which is part of the story rather than a footnote. Every fleet-scale number in the underlying research comes from a simulation substrate, and the proofs do not change that. And the novelty claim is bounded: after searching arXiv, Mathlib, Isabelle's Archive of Formal Proofs, and the adjacent formalization projects I could find, I believe this is the first machine-checked formalization of this chain, or of its components — conformal prediction validity, e-value calibration, e-BH — in any proof assistant. "As far as I could find" is the strongest form of that claim I can defend.
...
The prover also formalized a result that cuts against the system. Per-round validity does not survive accumulation: when units carry persistent quirks, multiplying their per-round e-values across rounds inflates the null mean strictly above 1, no matter how exact each round is. That limitation was discovered by simulation earlier in the program; it is now a machine-checked theorem sitting next to the guarantee it constrains. A verification effort that only proves the flattering statements is marketing. This one proved the unflattering one too.
...
Two of these deserve their mechanisms spelled out, because they generalize.
...
Why did the numerical validation miss all five? Because of an asymmetry that I think is the general lesson of this exercise. Numerical validation runs against the shipped implementation, which instantiates the objects you intend: summable weights, measurable ranks, thresholds between 0 and 1, a genuinely independent random number generator. There is no way to "run" the formal text itself. So the formal statements had two guardians — simulation, which was blind to them by construction, and the type checker, which verifies that a statement is well-formed, not that it is true or even satisfiable. The scaffold's own header contained the sentence "a wrong statement fails loudly at build." All five of these elaborated cleanly. The only tool that reads the statement you actually wrote, rather than the one you meant, is the proof obligation.

If you want the one-line version: simulation checks your intent; proving checks your text; and the guarantee you publish is the text.

## Is this new mathematics?

No, and the distinction is worth keeping sharp because it is where claims like this usually inflate.
