...
The prover also formalized a result that cuts against the system. Per-round validity does not survive accumulation: when units carry persistent quirks, multiplying their per-round e-values across rounds pushes the healthy-case average (the null mean) strictly above 1, no matter how exact each round is. That limitation was discovered by simulation earlier in the program; it is now a machine-checked theorem sitting next to the guarantee it constrains.
