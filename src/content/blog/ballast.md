...
That is what's real. Here is what isn't. F3, junk-resistance (an honest reporter beats a junk reporter, margin +60.70), and F4, incentive efficacy (over-reservation drops 0.527 → 0.150, a −71.6% reduction, while utilization rises 0.753 → 0.850 rather than collapsing into under-reservation), both passed in simulation only. No real organization has run Ballast's incentive loop. The claim that scoring reservations changes reservation behavior is a model of an org, not an observation of one.
